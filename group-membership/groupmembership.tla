...
CONSTANT Devices, \* Symmetry set of devices such as `{d1, d2, d3}'.
         Creator, \* Device that created the chat.
         MaxQueue \* Bound on the queue size to keep the model bounded.

VARIABLES Members, \* Map from device to the set of devices it thinks are members of the group 
          Queues \* FIFO queues of messages sent between devices.
